...
## Conventions
- CRITIQUE : `pnpm` pour le frontend, `uv` pour Python (JAMAIS pip, JAMAIS npm)
- Langue : français pour les échanges, anglais pour le code et l'UI
- Docstrings au style Google sur toutes les fonctions Python
- Pas de polling sur les hooks — utiliser staleTime + invalidation après les mutations
- Couleurs des activités centralisées dans `lib/utils.ts` — jamais dupliquer dans les composants
- La clé API (`GARMIN_API_KEY`) est côté serveur uniquement, jamais exposée au client
- Le frontend appelle le proxy `/api/garmin/*` qui transfère vers FastAPI avec les en-têtes d'authentification
- Le schéma Prisma contient les tables auth uniquement — les tables Garmin ne sont PAS dans Prisma (pour éviter que `db push` ne les supprime)
